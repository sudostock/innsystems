...
//*** added some variables
   ...
   //***     return (1 / (1 + Math.exp(-1.0 * sum)));
    ...
    /** Takes the data set and normalizes it by dividing each number by 1000
     *
     *  @param dataA array of the data to be normalized(this includes input and answers)
     *
     *  @return array of the data after it has been normalized */
    public double[] normalizeData(double dataA[]) {
        int dataL = dataA.length;
        //double input[] = new double[dataL];
        
        for(int i = 0; i < dataL; i++) {
   //***         dataA[i] = (dataA[i]/1000);
        }
        return dataA;
